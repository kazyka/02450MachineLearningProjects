...
for train_index, test_index in CV:
    ...
    for i in range(0,l1_max_nodes):
    	dnn_classifier = nn.MLPRegressor(solver='adam',alpha=1e-4,hidden_layer_sizes=(i+1,),random_state=0,max_iter=200,activation='relu')
    	dnn_classifier.fit(X_train, y_train)
    	y_est = dnn_classifier.predict(X_test)
    	errors[k,i] = np.dot((y_est-y_test).astype(float),(y_est-y_test).astype(float))/len(y_test)
